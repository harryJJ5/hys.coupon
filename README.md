...
- `hys.coupon`：项目编码 / artifactId / 默认应用名
- `coupon`：项目展示名称
- `com.hys.coupon`：Java 包名，例如 `com.company.springbootdemo`
- `com/hys/coupon`：Java 包路径，例如 `com/company/springbootdemo`
...
```text
JDBC URL: jdbc:h2:mem:hys.coupon_db;MODE=MYSQL;DATABASE_TO_LOWER=TRUE;DB_CLOSE_DELAY=-1;DB_CLOSE_ON_EXIT=FALSE
User Name: sa
Password: 空
```
...
2. ZIP 模板渲染后，`src/main/java/com/hys/coupon` 会自动变成真实包路径
